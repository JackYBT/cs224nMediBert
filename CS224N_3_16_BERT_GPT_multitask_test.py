...
class CustomModel(torch.nn.Module):
  ...
  def forward(self, encoder_input_ids=None, encoder_attention_mask=None, decoder_input_ids=None, decoder_attention_mask=None,departmentlabels=None, validation=False):
    ...
    return classificationLogits.cpu().detach().argmax(dim=1).numpy(), departmentlabels_indices.cpu().numpy(), hidden_states, seqoutputs, classificationAccuracy, classificationLoss
...
# def classification_eval(model, validation_loader, device):
#     total_accuracy = 0
...
#         total_accuracy += classificationAccuracy.item()

#     return total_accuracy / len(validation_loader)
...
def validation(model, validation_loader, tokenizer, device):
    ...
    with torch.no_grad():
        for curIndex, data in enumerate(validation_loader, 0):
            ...
            outputs, targets,_,_,curClassificationAccuracy,_ = model(encoder_input, encoder_input_mask, decoder_input, mask_decoder_input, departmentlabels, validation=True)
            totalClassificationAccuracy += curClassificationAccuracy
            fin_targets.extend(targets)
            fin_outputs.extend(outputs)
    return fin_outputs, fin_targets, curClassificationAccuracy/len(validation_loader)
            # # gts, res = eval_prep(model, decoder_input, curIndex, tokenizer, generator, encoder_input, encoder_input_mask)
            # gts = generate_gts(curIndex, decoder_input, tokenizer)
            # sample_output = torch.argmax(seqoutputs.logits, dim = -1).squeeze()
            # res = generate_gts(curIndex, sample_output, tokenizer)
...
#------------------------START TRAINING-------------------
    ...
    for epoch in range(num_epochs):
        ...
        for update_count, data in enumerate(training_loader):
            ...
            _, _, enc_hs, outputs, classificationLoss, classificationAccuracy= model(encoder_input, mask_encoder_input, decoder_input, mask_decoder_input, departmentlabels)
...
destinationFileName = '3_19_bert_gpt_multitask_allpatient_trained_with_pretrained_GPT'
...
def main():
    ...
    outputs, targets, accuracy = validation(model, testing_loader, decoder_tokenizer, device)

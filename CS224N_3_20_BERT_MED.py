...
class CustomModel(torch.nn.Module):
  ...
  def forward(self, encoder_input_ids=None, encoder_attention_mask=None, decoder_input_ids=None, decoder_attention_mask=None,departmentlabels=None):
    ...
    return hidden_states, classificationLoss, classificationAccuracy
...
#------------------------START TRAINING-------------------
    ...
    for epoch in range(num_epochs):
        ...
        for update_count, data in enumerate(training_loader):
            ...
            enc_hs, classificationLoss, classificationAccuracy= model(encoder_input, mask_encoder_input, decoder_input, mask_decoder_input, departmentlabels)
            
            curLoss = classificationLoss
            curLoss.backward()
            ...
            if update_count % 250 == 0:
                ...
                times = 0
                total_losses = 0
                wandb.log({'current classificationLoss': classificationLoss, 'current totalLoss': curLoss, 'current classificationAccuracy': classificationAccuracy})
                wandb.log({'cumulative classificationLoss': epoch_classification_loss / (update_count + 1), 'cumulative classificationAccuracy': epoch_classification_accuracy / (update_count + 1)})
...
destinationFileName = '3_20_bert_med'
